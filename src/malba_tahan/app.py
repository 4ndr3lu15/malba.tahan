"""the textual terminal ui for malba tahan."""
...
class ResultScreen(ModalScreen[None]):
    """win / lose overlay shown when the round ends."""
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="result-box", classes="won" if self._won else "lost"):
            ...
            yield Button("play again", variant="success", id="play-again")
...
class MalbaTahanApp(App[None]):
    """a balance-puzzle toy: find the one heavier ball in at most two weighings."""
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static("malba tahan — 8 weights of gold", id="title")
        yield Static(
    ...
    def on_mount(self) -> None:
        ...
        self.sub_title = "the man who counted — 8 weights of gold"
    ...
    def refresh_view(self, message: str) -> None:
        ...
        self.query_one("#identify", Button).disabled = g.weighings_taken == 0
    ...
    def action_weigh(self) -> None:
        ...
        if self.game.weighings_left == 0:
            self._enter_identify(
                f"{verdict}  no weighings left — choose the heavy ball!"
            )
        else:
    ...
    def _make_guess(self, ball: int) -> None:
        ...
        self.push_screen(ResultScreen(won, message), restart)
